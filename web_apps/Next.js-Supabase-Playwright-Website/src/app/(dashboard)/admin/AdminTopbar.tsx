...
const VIEW_AS = [
  ...
  { href: "/applicant/dashboard", label: "Applicant" },
] as const;
...
export function AdminTopbar() {
  ...
  return (
    ...
    >
      <Link href="/admin" className="flex items-center" aria-label="Intrastack admin home">
        <Image
      ...
      <div className="ml-auto flex items-center gap-2">
        {VIEW_AS.map((v) => (
          <Button key={v.href} asChild size="sm" className="bg-brand-500 text-white hover:bg-brand-500">
            <Link href={v.href}>View {v.label}</Link>
          </Button>
        ))}
        <Button variant="ghost" size="icon" className="ml-2 text-zinc-500 hover:bg-zinc-200 hover:text-zinc-800">
